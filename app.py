...
if st.button("Predict"):
    saved_preprocessor = joblib.load("preprocessor.joblib")
    # Ensure x_new has the same structure as X_train before transforming
    x_new = pd.DataFrame(dict(
        airline=[airline],
        date_of_journey=[doj],
        source=[source],
    ...
    # Transform x_new with the preprocessor
    x_new_pre = saved_preprocessor.transform(x_new)

    # Ensure x_new_pre has the same columns as expected by the XGBoost model
    print(x_new_pre.shape)  # Check the shape
    print(x_new_pre.dtypes)  # Check the data types
